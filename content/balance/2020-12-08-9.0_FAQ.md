...
9.2 PTR is here and brings some additions to every spec. Everyone is getting their covenant legendary equipable for free, which means all covenant abilities are enhanced baseline. It is to note that this does mean more choices since you can still only choose one legendary freely.\
On top of this the first iteration of every set bonus has been revealed, ours being the following:

**2-Set -  {{< spell Umbral Embrace "365473" >}}:**\
...
**4-Set -  {{< spell Umbral Infusion "363497" >}}:**\
While in an Eclipse, the cost of  Starsurge and  Starfall is reduced by 20%.\
*Notes:* This stacks multiplicatively with {{< spell Timeworn Dreambinder "339949" >}} and does not reduce progress per Astral Power spend for 
{{< spell Primordial Arcanic Pulsar "338668" >}}.

### Analysis:

With how the sets currently work, they will heavily favour one legendary, 
{{< spell Primordial Arcanic Pulsar "338668" >}} since both set bonuses heavily synergize with it.\
The 2-Set Umbral Embrace procs its Fury of Elune every time Pulsar is procced which in turn resets your Eclipse and allows for even more Lunar Eclipses. Even when proccing Pulsar inside a running CA/Inc the FoE will spawn.\
...
Looking at covenants, the baseline covenant legendary and set bonuses favor Venthyr and Kyrian the most since their covenant legendaries are far superior to the Night Fae and Necrolord ones. Venthyr and Kyrian both gain about 20% dps from their respective legendaries while Night Fae gain 3% at best and a damage loss at worst. In the scenario that acquiring the baseline covenant legendary were to take a long time Night Fae would still be competitive until that point.
