...
The 9.2 additions to every spec's arsenal are the following: Tier sets and a second equipable legendary. For us specifically, in their current form the tier set bonuses look like this:

	2-Set -  Umbral Embrace - Entering Lunar Eclipse creates a  Fury of Elune at 25% effectiveness that follows your current target for 8 sec.
	4-Set -  Umbral Infusion - While in an Eclipse, the cost of  Starsurge and  Starfall is reduced by 20%.

# Results:

With how the sets currently work, they will heavily favour one legendary, Primordial Arcanic Pulsar. 
Every time Pulsar gets used, a FoE will proc from Umbral Embrace, even if one is currently inside a CA window. 
Every time AP is being spent, by either casting Starfall or Starsurge, the original cost (50 or 30 respectively) will be added towards the Pulsar pool instead of the reduced 40 or 24. What this means is that we will be able to get higher pulsar uptime from the 4piece. 

If you were to think covenant wise, the biggest winners here would be Venthyr and Kyrian in an endgame scenario and Night Fae in a tier start scenario if the 2 and 4 piece sets were acquired before the 2nd legendary (We currently have no information when and how the 2nd legendary is acquired.). This is mostly because of the fact that Night Fae was already running Pulsar and their second legendary unlock (Celestial Spirits) is very bad for balance. On the other hand, Venthyr and Kyrian are both using their respective covenant legendaries already which means that they will add pulsar on top. Purely sim wise on a 5min patchwork 1target profile, night fae only sims around 300dps lower if compared to Venthyr at 4piece set bonus and only one legendary, which would make it playable over Venthyr depending on fight.  

The question that everyone reading this has in mind right now is something along the lines of: "Hey, all this is great information and stuff but what covenant do I actually go?". The answer here is simple. All of them. In case Venthyr and/or Kyrian get nerfed, you can still go Night Fae. Just get renown 80 on all these covenants and you will be safe. Covenants will most likely be chosen on a boss by boss basis. I would recommend holding necrolord on renown 1 in case Instructor's Divine Bell is worth farming again (see the first pin in the dreamgrove discord for more information).  


## 9.1.5 Changes

Nothing changed for Balance Druids in 9.1.5.
...
- {{< spell 340540 "Ursine Vigor" >}}: Really good if you will ever go into bear form to soak an ability. Especially good on progression and if you will ever press Heart of the Wild while being talented into Guardian Affinity. Examples: Surviving a bomb+beam overlap on Fatescripe or Sylvanas' arrows.
...
- {{< spell 340543 "Innate Resolve" >}}: Mediocre value since it will only have value when Well-Hones Instincts procs or you are already actively healing yourself. In both cases you are very likely already save.
...
In M+: {{< item 355315 "Sinful Hysteria" >}}
