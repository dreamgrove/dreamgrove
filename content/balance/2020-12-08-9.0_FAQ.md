...
## [23. What Trinkets should I use?]

</div>


**DISCLAIMER: All of the trinkets in these combinations are examples. They are all dependant on Ilvl, the best bet would be to sim all of them using either Topgear, Droptimiser or Gear Compare.**
...
**Double on Use:**
This combination can prove to be the strongest by far and excels in single target. The only problem is that it requires specific trinkets and can be heavily devalued by fight timers and volatility. Meaning it should not be used in unpredictable scenarios or situations where you have to either use your cooldowns immediately or you would lose a {{< spell 323764 "Convoke" >}} by playing double on use.
Refer to [#17](#double-on-use) to learn more about how to play with this combination.

Usable Trinkets:
- Slot1 (EO): Empyreal Ordnance is the only trinket slotable, as it is the only reason this build exists.
- Slot2 (another on use): Inscrutable Quantum Device, Instructor's Divine Bell and others.
...
Usable trinkets:
- Slot1 (Equip): Cabalist's Hymnal, Unbound Changeling, Insignia of Alacrity and others.
- Slot2 (Use): Inscrutable Quantum Device, Instructor's Divine Bell, Soulletting Ruby and others. 
...
Usable trinkets:
- Slot1+2 (Equip): Cabalist's Hymnal, Unbound Changeling, Insignia of Alacrity and others.

**Notable trinkets:**
Inscrutable Quantum Device: Even though this trinket has the potential to be incredibly powerful, without being given the necessary environment, it can be worth very little and can be replaced by most other trinkets. Refer to [#11](#iqd) to find out more about how this trinket works.
...
Empyreal Ordnance: The enabler of the double on use combination is one of the more special trinkets, having two parts: a damaging and a buffing one. After the damaging part finishes, the buffing part of the trinket comes back to the caster, meaning that it has two durations. Due to this it should mostly only be used in combination with anotheron use trinket and not as a standalone one.
...
Unbound Changeling: As the name suggests, this trinket gains most of its value from the fact that it is convertable into a mastery proc trinket.

Insigina of Alacrity: Another great equip trinket which has both positives and negatives. The negatives being that the trinket has a lower stat budget and the positives being that it can be acquired through pvp at 226 ilvl.

Soulletting Ruby: Although this trinket has a bit higher stat budget than others, the fact that we get crit instead of other stats inside our cds is not that helpful, as most of our damage inside our cds is frontloaded into out boat window. 
