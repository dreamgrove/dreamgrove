...
draft: false
authors: ["Faide"]
summary: 'A thorough discussion of diminishing returns'

---

> Author's note: I originally wrote this article back in August 2018, and I have since retired from both theorycrafting and WoW in general. To my knowledge the information in this article is still accurate and relevant, although many of the numeric values have changed since I wrote it. I hope you still find it insightful and/or valuable!

I've seen a lot of questions surface recently about diminishing returns &mdash; what they are, how they are (or aren't) related to damage reduction, and what stats are (or aren't) affected. Let's talk about it.
...
![Marginal Gain of Adding 1 Stat](/static/images/diminishing-returns/marginal-gains.png)

Many people understand this intuitively as "stat X gets worse the more you have of it". This is inherent in the way that stat ratings are calculated, and all stats are affected equally by this phenomenon. 
...
Dodge chance gained from gear is subject to a diminishing returns penalty. The full Dodge formula is longer (and not all Dodge is penalized), but only this part of the equation is important to the current discussion.
...
![Effects of Diminishing Returns on Dodge](/static/images/diminishing-returns/dodge-diminishing-returns.png)
...
![Damage Reduction from Armor against a Level 123 Attacker](/static/images/diminishing-returns/damage-reduction-from-armor.png)

In fact: any stat that is converted using a function with the shape $$f(x) = \frac{x}{Ax + C}$$ where A and C are constant, positive numbers will be subject to diminishing returns.
...
As such, gaining 1% damage reduction is **50 times more valuable** when you already have 98% damage reduction than it is when you have 0% damage reduction. Rather than diminishing returns, damage reduction has "increasing returns" &mdash; every percent of damage reduction you gain is more valuable than the last.
...
For example: if you have 100 health and 50% damage reduction, your effective health is $$\frac{100}{1 - 0.5} = 200$$, meaning that it would take 200 damage (before mitigation) to kill you.
...
![Effective Health vs Damage Reduction](/static/images/diminishing-returns/ehp-damage-reduction.png)
...
Recall that the Armor to damage reduction formula is $$damageReduction = \frac{armor}{armor + K}$$, which inherently suffers diminishing returns when expressed as damage reduction. Let's try and express it as effective health by substituting the Armor formula into the EHP formula.
...
![Effective Health vs Armor Rating](/static/images/diminishing-returns/ehp-armor.png)

The same is true for Dodge/Parry, Block, and Stagger &mdash; any stat that provides damage reduction and is subject to a diminishing returns penalty will grant linear returns to effective health, and therefore to survivability.
...
Let's walk through the math. The formula for converting Versatility rating to percent damage reduction is $$damageReduction = \frac{versRating}{ratingPerPercent * 2 * 100}$$, where ratingPerPercent is 85 at level 120.
...
![Effective Health vs Versatility Rating](/static/images/diminishing-returns/ehp-versatility-rating.png)

As you can see, Versatility actually has *increasing* returns with respect to effective health. This is what we mean when we say "Versatility gets better the more you have of it", and why it makes for such a strong stat for survivability.
